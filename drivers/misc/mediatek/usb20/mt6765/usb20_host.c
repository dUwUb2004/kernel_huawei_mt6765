...
#endif
#endif
		if (g_otg_bigcurr_disable == false)
...
static int otg_iddig_probe(struct platform_device *pdev)
{
	...
	if (iddig_eint_num < 0)
		return -ENODEV;
	if (of_property_read_bool(node, "otg_bigcurr_disable"))
		g_otg_bigcurr_disable = true;
	else
		g_otg_bigcurr_disable = false;
	DBG(0, "g_otg_bigcurr_disable = %d\n", g_otg_bigcurr_disable);
